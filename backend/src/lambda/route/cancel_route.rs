...
async fn cancel_route(
    ...
    let body = parse_body::<CancelRouteRequest>(&event)?;
    ...
    if !check_route_exists(ssm_client).await? {
        return json_message(200, "No route is set");
    }

    let route = Route::cancelled(body.message);
...
async fn check_route_exists(ssm_client: &ssm::Client) -> Result<bool, Error> {
    ...
    let Some(param) = ssm_resp.parameter else {
        return Ok(false);
    };
    let route_data = param.value.ok_or("No value found")?;
    let current_route: Route = serde_json::from_str(&route_data)?;
    Ok(current_route.is_ready())
}
...
fn build_email_body(route: &Route, recipient: &EmailRecipient) -> String {
    include_str!("../../templates/cancelled.html")
