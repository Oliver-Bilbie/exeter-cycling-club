            ...
            ? {
                min:
                  width > WIDTH_THRESHOLD
                    ? "1150px"
                    : `${1700 - (WIDTH_THRESHOLD - width) / 2}px`,
              }
            : { min: "700px" }
        }
      >
        ...
        {(status === "cancelled" || status === "unavailable") && (
          <Box align="center" margin="large">
            <Heading textAlign="center" margin="none">
              {status === "cancelled" && "This week's ride has been cancelled"}
              {status === "unavailable" &&
        ...
        {user.admin && (
          ...
          >
            <Link to={"/select"}>
              <Button label="Set new route" alignSelf="center" primary />
            </Link>
            {status === "ready" && (
              <Link to={"/cancel"}>
                <Button label="Cancel route" alignSelf="center" primary />
              </Link>
            )}
          </Box>
        )}
