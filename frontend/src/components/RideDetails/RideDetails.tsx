...
const RideDetails: React.FC<RideDetailsProps> = ({
  ...
  useEffect(() => {
    if (routeData.gpx !== "") {
      ...
      const routeElevation = gpx.tracks[0].points.map((p): number => p.ele);
  ...
  return (
    ...
    >
      <Box
        width={width > WIDTH_THRESHOLD ? `${(3 * width) / 7}px` : `${width}px`}
      ...
      >
        {loading ? (
          ...
          <Map
            ...
            center={route.center}
            zoom={width > WIDTH_THRESHOLD ? 11 : width > 500 ? 10 : 9}
      ...
      >
        ...
        <RangeInput value={highlight.index} onChange={updateHighlight} />
      </Box>
    </Box>
  );
};
