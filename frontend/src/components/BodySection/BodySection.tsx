...
const BodySection: React.FC<BodyProps> = ({
  ...
  return (
    <Box
      background={color}
      direction={slim ? "column-reverse" : reverse ? "row-reverse" : "row"}
      pad="large"
      gap="large"
    ...
    >
      ...
      <Box direction="column" justify="center" gap="medium">
        <Heading margin="small" responsive={false}>
          {title}
        </Heading>
        {text.map((paragraph: string, index: number): React.ReactElement => {
